...
function main() {
    console.log('🚀 Bathroom-Specific Causal Analysis Engine v1.0')
    console.log('Target Area: 卫生间 (Leak Area Code: 2)\n')
    ...
    // 4. Calculate ATE for Onsite Rate
    const results: TagResult[] = tags.map(tag => {
        ...
        const calcRate = (agents: typeof agentStats) => {
            ...
            return agents.reduce((sum, a) => sum + (a.onsite_deals / a.total_deals), 0) / agents.length
        }

        const treatedOnsiteRate = calcRate(treatedAgents)
        const controlOnsiteRate = calcRate(controlAgents)
    ...
    // 5. Output results
    results.sort((a, b) => b.ateOnsite - a.ateOnsite)

    console.log('✅ CAUSAL IMPACT ON ONSITE RATE (BATHROOM ONLY):')
    console.log('Tag Code             | Name                 | Onsite ATE | Treat | Ctrl')
    console.log('---------------------+----------------------+------------+-------+-----')
    results.filter(r => r.treatedAgents >= 2).slice(0, 15).forEach(r => {
        const onsiteColor = r.ateOnsite > 0 ? '\x1b[32m' : '\x1b[31m'
        console.log(`${r.code.padEnd(20)} | ${r.name.padEnd(20)} | ${onsiteColor}${(r.ateOnsite * 100).toFixed(2).padStart(8)}%\x1b[0m | ${r.treatedAgents.toString().padStart(5)} | ${r.controlAgents.toString().padStart(4)}`)
    })

    console.log('\n❌ NEGATIVE IMPACT (BATHROOM ONLY):')
