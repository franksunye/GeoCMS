...
export function KnowledgeStatsCard({ knowledge }: KnowledgeStatsCardProps) {
  ...
  const getTimeliness = (days: number) => {
    if (days <= 30) return { label: 'Latest', color: 'text-green-600' }
    if (days <= 90) return { label: 'Recent', color: 'text-blue-600' }
    if (days <= 180) return { label: 'Needs Update', color: 'text-yellow-600' }
      ...
      <div className="flex items-center space-x-2">
        ...
        <div>
          ...
          <div className={`text-sm font-medium ${timeliness.color}`}>
            {timeliness.label}
            <span className="text-xs text-gray-500 ml-1">({daysSinceUpdate} days)</span>
          </div>
        </div>
      </div>
